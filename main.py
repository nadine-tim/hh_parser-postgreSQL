...
def main():
    ...
    # print(f'все вакансии: {vacancy_list}')

    db_manager.insert_vacancies_db(vacancy_list)

    print("Данные по вакансиям были сохранены в БД")

    line2 = "Для вывода статистики по вакансиям, выберите опции: "
    ...
    user_input = input("Введите опции: ")
    while user_input != "exit":
        if user_input == '1':
            result = db_manager.get_companies_and_vacancies_count()
            for data in result:
                print(data)
                # print(f"В компании {data['employer']}: {data['count']} вакансий")
        elif user_input == '2':
            result = db_manager.get_all_vacancies()
            for data in result:
                print(f"В компании: {data['employer']}, вакансия: {data['vacancy']}, зарплата: {data['salary_to']}, ссылка: {data['url']} ")
        elif user_input == '3':
